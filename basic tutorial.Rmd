...
data.cum <- NULL
log.normal.sigma <- function(Median, P, num) {
  if (Median > num) {
    P <- 2 * (0.5 - P/2) - 1
  } else {
    P <- 2 * (0.5 + P/2) - 1
  }
  sigma <- (log(num)-log(Median))/(sqrt(2)*invErf(P))
  return(sigma)
}
data.infection.median.death <- p.death$infection$layers[[2]]$data
data.infection.death <- p.death$infection$data
data.infection.median.bed <- p.beds$infection$layers[[2]]$data
data.infection.bed <- p.beds$infection$data
cum.infection <- function(data.infection.median, data.infection) {
  for (i in 1:nrow(data.infection.median)) {
  d <- data.infection.median$date[i]
  data.infection.temp <- filter(data.infection, date == d)
  m <- as.numeric(data.infection.median$median[i])
  s <- 1
  for (p in c(30, 60, 95)) {
    data.infection.temp.level <- filter(
      data.infection.temp, level == p)
    s <- s * log.normal.sigma(
      Median = m, P = p/100, 
      num = as.numeric(data.infection.temp.level$lower)
      )
    s <- s * log.normal.sigma(
      Median = m, P = p/100, 
      num = as.numeric(data.infection.temp.level$upper)
      )
  }
  s <- s^(1/6)
  mu <- log(m)
  data.cum <- rbind(
    data.cum, rlnorm(10000, meanlog = mu, sdlog = s))
  }
  for (i in 2:nrow(data.cum)) {
  data.cum[i,] <- data.cum[i-1,] + data.cum[i,]
}
  data.infection.median$low95 <- 0
  data.infection.median$low60 <- 0
  data.infection.median$low30 <- 0
  data.infection.median$up30 <- 0
  data.infection.median$up60 <- 0
  data.infection.median$up95 <- 0
  data.infection.median$M <- 0
  for (i in 1:nrow(data.infection.median)) {
    cum <- sort(data.cum[i,])
    data.infection.median$low95[i] <- cum[250] / 1000000
    data.infection.median$low60[i] <- cum[2000] / 1000000
    data.infection.median$low30[i] <- cum[3500] / 1000000
    data.infection.median$up30[i] <- cum[6500] / 1000000
    data.infection.median$up60[i] <- cum[8000] / 1000000
    data.infection.median$up95[i] <- cum[9750] / 1000000
    data.infection.median$M[i] <- median(cum) / 1000000
  }
  return(data.infection.median)
}
cum.infection.death <- cum.infection(
  data.infection.median.death, data.infection.death)
cum.infection.bed <- cum.infection(
  data.infection.median.bed, data.infection.bed)


ggplot() + 
  geom_ribbon(
    aes(ymin=low30, ymax=up30, x=date), data = cum.infection.death,
    alpha = 0.8, fill = "#236585") + 
  geom_ribbon(
    aes(ymin=low60, ymax=up60, x=date), data = cum.infection.death,
    alpha = 0.7, fill = "#1A7796") + 
  geom_ribbon(
    aes(ymin=low95, ymax=up95, x=date), data = cum.infection.death,
    alpha = 0.6, fill = "#447D99") + 
  geom_ribbon(
    aes(ymin=low30, ymax=up30, x=date), data = cum.infection.bed,
    alpha = 0.8, fill = "#4D9C70") + 
  geom_ribbon(
    aes(ymin=low60, ymax=up60, x=date), data = cum.infection.bed,
    alpha = 0.7, fill = "#8DBFA3") + 
  geom_ribbon(
    aes(ymin=low95, ymax=up95, x=date), data = cum.infection.bed,
    alpha = 0.6, fill = "#D2E6DB") + 
  scale_y_continuous("Cumlative Cases (Million)") +
  annotate(
    x = as.Date("2020-07-15"), xend = as.Date("2020-07-15"), 
    y = 3.695, yend = 3.886, 
    geom = "segment", size = 1, colour = "red") +
  annotate(
    x = as.Date("2020-07-13"), xend = as.Date("2020-07-17"), 
    y = 3.695, yend = 3.695, 
    geom = "segment", size = 0.5, colour = "red") +
  annotate(
    x = as.Date("2020-07-13"), xend = as.Date("2020-07-17"), 
    y = 3.886, yend = 3.886, 
    geom = "segment", size = 0.5, colour = "red") +
  annotate(
    x = as.Date("2020-07-10"), xend = as.Date("2020-07-20"), 
    y = 3.822, yend = 3.822, 
    geom = "segment", size = 0.8, colour = "red") +
  geom_line(
    aes(date, M), data = cum.infection.death, 
    lwd = 1, colour = "black") +
  geom_line(
    aes(date, M), data = cum.infection.bed, 
    lwd = 1, colour = "black") +
  scale_x_date(date_breaks="3 week",date_labels="%d %b") +
  theme_bw() +  
  theme(
