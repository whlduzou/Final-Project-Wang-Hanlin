...
plot.cases <- function(
  ...
  data <- filter(data, date < as.Date(end))
  data <- filter(data, date > as.Date(start))
...
```{r fig.cap="Simulation infection until Oct 1st, 2020", fig.align='center'}
p.beds$infection <- p.beds$infection + guides(fill=F) 
cowplot::plot_grid(
...
```{r reproduction_number, fig.cap="Simulation reproduction number until Oct 1st, 2020", fig.align='center'}
...
p.death$rt <- add.react(p.death$rt)
...
The yellow area means the result of r_t of the REACT group

```{r observation, fig.cap="Fitness of obeservation data until Oct 1st, 2020", fig.align='center'}
...
```{r cases, fig.cap="Cases, beds and death data until Oct 1st, 2020", fig.align='center'}
...
p1 <- ggplot(
  data = new.data, 
  mapping = aes(x = date, y = num, colour = type)) + 
  geom_line(lwd = 1) + 
  scale_x_date(date_breaks="3 week",date_labels="%d %b %Y") +
  theme_bw() +  
  theme(
...
new.dataa <- new.data[new.data$date < as.Date(time.end),]
row.names(new.dataa) <- 1:nrow(new.dataa)
new.dataa <- new.dataa[new.dataa$date > as.Date(time.start),]
p2 <- ggplot(
  data = new.dataa, 
  mapping = aes(x = date, y = num, colour = type)) + 
  geom_line(lwd = 1) + 
