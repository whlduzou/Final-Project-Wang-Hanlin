...
cum.infection.plot <- function(data.infection.median, data.infection) {
  ...
  for (i in 1:nrow(data.infection.median)) {
    cum <- sort(data.cum[i,])
    data.infection.median$low95[i] <- cum[250]
    data.infection.median$low60[i] <- cum[2000]
    data.infection.median$low30[i] <- cum[3500]
    data.infection.median$up30[i] <- cum[6500]
    data.infection.median$up60[i] <- cum[8000]
    data.infection.median$up95[i] <- cum[9750]
  }
...
ggplot() + 
  geom_line(
    aes(date, median), data = cum.infection.death, 
    lwd = 1, colour = "black") +
  geom_ribbon(
