...
p.beds$infection + geom_line(
  ...
  lwd = 1, data = new.data.temp) +
  scale_y_continuous(
    sec.axis = sec_axis(~ ./4, name = "Observe")
    ) +
  theme(legend.position = "right")
```
...
data.bed.infection.median <- p.beds$infection$layers[[2]]$data
data.bed.infection.median$median <- 
  data$cases / data.bed.infection.median$median * 100 + 0.01
data.bed.infection <- p.beds$infection$data

data.bed.infection.30 <- filter(
  data.bed.infection, level == "30"
  )
data.bed.infection.30$lower <- 
  data$cases / data.bed.infection.30$lower * 100 + 0.01
data.bed.infection.30$upper <- 
  data$cases / data.bed.infection.30$upper * 100 + 0.01
data.bed.infection.60 <- filter(
  data.bed.infection, level == "60"
  )
data.bed.infection.60$lower <- 
  data$cases / data.bed.infection.60$lower * 100 + 0.01
data.bed.infection.60$upper <- 
  data$cases / data.bed.infection.60$upper * 100 + 0.01
data.bed.infection.95 <- filter(
  data.bed.infection, level == "95"
  )
data.bed.infection.95$lower <- 
  data$cases / data.bed.infection.95$lower * 100 + 0.01
data.bed.infection.95$upper <- 
  data$cases / data.bed.infection.95$upper * 100 + 0.01

sampling.beds <- ggplot() + 
  geom_line(
    aes(date, median), data = data.bed.infection.median, 
    lwd = 1, colour = "#102631") +
  geom_ribbon(
    aes(ymin=upper, ymax=lower, x=date), 
    data = data.bed.infection.30,
    alpha = 0.6, fill = "#236585") + 
  geom_ribbon(
    aes(ymin=upper, ymax=lower, x=date), 
    data = data.bed.infection.60,
    alpha = 0.4, fill = "#1A7796") + 
  geom_ribbon(
    aes(ymin=upper, ymax=lower, x=date), 
    data = data.bed.infection.95,
    alpha = 0.2, fill = "#447D99") + 
  geom_hline(
    aes(yintercept=100), 
...
data.death.infection.median <- p.death$infection$layers[[2]]$data
data.death.infection.median$median <- 
  data$cases / data.death.infection.median$median * 100 + 0.01
data.death.infection <- p.death$infection$data

data.death.infection.30 <- filter(
  data.death.infection, level == "30"
  )
data.death.infection.30$lower <- 
  data$cases / data.death.infection.30$lower * 100 + 0.01
data.death.infection.30$upper <- 
  data$cases / data.death.infection.30$upper * 100 + 0.01
data.death.infection.60 <- filter(
  data.death.infection, level == "60"
  )
data.death.infection.60$lower <- 
  data$cases / data.death.infection.60$lower * 100 + 0.01
data.death.infection.60$upper <- 
  data$cases / data.death.infection.60$upper * 100 + 0.01
data.death.infection.95 <- filter(
  data.death.infection, level == "95"
  )
data.death.infection.95$lower <- 
  data$cases / data.death.infection.95$lower * 100 + 0.01
data.death.infection.95$upper <- 
  data$cases / data.death.infection.95$upper * 100 + 0.01

sampling.death <- ggplot() + 
  geom_line(
    aes(date, median), data = data.death.infection.median, 
    lwd = 1, colour = "#102631") +
  geom_ribbon(
    aes(ymin=upper, ymax=lower, x=date), 
    data = data.death.infection.30,
    alpha = 0.6, fill = "#236585") + 
  geom_ribbon(
    aes(ymin=upper, ymax=lower, x=date), 
    data = data.death.infection.60,
    alpha = 0.4, fill = "#1A7796") + 
  geom_ribbon(
    aes(ymin=upper, ymax=lower, x=date), 
    data = data.death.infection.95,
    alpha = 0.2, fill = "#447D99") + 
  geom_hline(
    aes(yintercept=100), 
    colour="#990000", linetype="dashed") +
